    ...
    Hegel.Session.run_hegel_test
      ...
      (fun tc ->
         with_metrics (fun () ->
           ...
           let key_gen =
             if mode = "non_basic" then Json_params.make_non_basic key_gen else key_gen
           in
           let val_gen = integers ~min_value:min_value_p ~max_value:max_value_p () in
           let val_gen =
             if mode = "non_basic" then Json_params.make_non_basic val_gen else val_gen
           in
