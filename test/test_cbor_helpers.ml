...
let test_roundtrip_string () =
  ...
  let decoded = decode (encode v) in
  Alcotest.(check string) "string round-trip" "hello world" (extract_string decoded)
;;
...
let test_roundtrip_bytes () =
  ...
  let decoded = decode (encode v) in
  Alcotest.(check string) "bytes round-trip" "\x01\x02\x03" (extract_bytes decoded)
;;
...
let test_nested_dict_with_list_values () =
  let v =
    `Map
      [ `Text "scores", `Array [ `Int 90; `Int 85; `Int 95 ]; `Text "name", `Text "test" ]
  in
  let decoded = decode (encode v) in
  let m = extract_dict decoded in
...
let check_failure msg f =
  ...
  (try
   ...
   | Failure m ->
     raised := true;
     Alcotest.(check bool) (msg ^ " contains type info") true (String.length m > 0));
  Alcotest.(check bool) (msg ^ " raised") true !raised
;;

let test_extract_int_wrong_type () =
  check_failure "int from text" (fun () -> ignore (extract_int (`Text "hi") : int))
;;

let test_extract_float_wrong_type () =
  check_failure "float from int" (fun () -> ignore (extract_float (`Int 42) : float))
;;
...
let test_extract_bytes_wrong_type () =
  check_failure "bytes from text" (fun () -> ignore (extract_bytes (`Text "hi")))
;;
...
let test_is_null_true () = Alcotest.(check bool) "null is null" true (is_null `Null)
...
let tests =
  ...
  ; Alcotest.test_case "round-trip negative int" `Quick test_roundtrip_negative_int
  ...
  ; Alcotest.test_case "round-trip empty string" `Quick test_roundtrip_empty_string
  ...
  ; (* Extractor errors *)
    Alcotest.test_case "extract_int wrong type" `Quick test_extract_int_wrong_type
  ; Alcotest.test_case "extract_float wrong type" `Quick test_extract_float_wrong_type
  ; Alcotest.test_case "extract_string wrong type" `Quick test_extract_string_wrong_type
  ; Alcotest.test_case "extract_bool wrong type" `Quick test_extract_bool_wrong_type
  ; Alcotest.test_case "extract_bytes wrong type" `Quick test_extract_bytes_wrong_type
  ; Alcotest.test_case "extract_list wrong type" `Quick test_extract_list_wrong_type
  ; Alcotest.test_case "extract_dict wrong type" `Quick test_extract_dict_wrong_type
  ; (* Extractor with null *)
