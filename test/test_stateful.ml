...
let stateful_rule_name_test () =
  ...
  Alcotest.(check (float 0.)) "default weight" 1.0 (S.Rule.weight rule);
  let weighted = S.Rule.create ~name:"my_rule" ~weight:2.5 ~step:(fun _tc s -> s) () in
  Alcotest.(check (float 0.)) "explicit weight" 2.5 (S.Rule.weight weighted)
;;
