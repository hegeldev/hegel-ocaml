...
let test_from_regex_schema () =
  ...
  | Some s ->
    ...
    let pat = Cbor_helpers.extract_string (List.assoc (`Text "pattern") pairs) in
    Alcotest.(check string) "pattern" "[a-z]+" pat;
    let fm = Cbor_helpers.extract_bool (List.assoc (`Text "fullmatch") pairs) in
...
let test_domains_max_length () =
  ...
  | Some s ->
    let pairs = Cbor_helpers.extract_dict s in
    let ml = Cbor_helpers.extract_int (List.assoc (`Text "max_length") pairs) in
...
let test_text_with_codepoint_range () =
  ...
  | Some s ->
    let pairs = Cbor_helpers.extract_dict s in
    let min_cp = Cbor_helpers.extract_int (List.assoc (`Text "min_codepoint") pairs) in
    let max_cp = Cbor_helpers.extract_int (List.assoc (`Text "max_codepoint") pairs) in
  ...
  | Some s ->
    ...
    (match cats with
     | `Array items ->
       ...
       Alcotest.(check (list string)) "exclude_categories" [ "Zs"; "Cs" ] strs
     | _ -> Alcotest.fail "expected array")
  | None -> Alcotest.fail "expected schema"
;;
...
let test_text_with_include_characters () =
  ...
  | Some s ->
    let pairs = Cbor_helpers.extract_dict s in
    let inc =
      Cbor_helpers.extract_string (List.assoc (`Text "include_characters") pairs)
    in
...
let test_text_with_exclude_characters () =
  ...
  | Some s ->
    let pairs = Cbor_helpers.extract_dict s in
    let exc =
      Cbor_helpers.extract_string (List.assoc (`Text "exclude_characters") pairs)
    in
...
let test_text_with_alphabet () =
  ...
  | Some s ->
    ...
    let cats = List.assoc (`Text "categories") pairs in
    (match cats with
     | `Array items -> Alcotest.(check int) "empty categories" 0 (List.length items)
     | _ -> Alcotest.fail "expected array")
  | None -> Alcotest.fail "expected schema"
;;
...
let tests =
  ...
  ; Alcotest.test_case "from_regex no fullmatch" `Quick test_from_regex_no_fullmatch
  ...
  ; Alcotest.test_case "text with codepoint range" `Quick test_text_with_codepoint_range
  ; Alcotest.test_case "text with categories" `Quick test_text_with_categories
  ; Alcotest.test_case
      "text with exclude_categories"
      `Quick
      test_text_with_exclude_categories
  ; Alcotest.test_case
      "text exclude_categories Cs present"
      `Quick
      test_text_exclude_categories_cs_already_present
  ; Alcotest.test_case
      "text with include_characters"
      `Quick
      test_text_with_include_characters
  ; Alcotest.test_case
      "text with exclude_characters"
      `Quick
