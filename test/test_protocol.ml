...
let test_roundtrip_simple () =
  with_socket_pair (fun reader writer ->
    let pkt = { stream_id = 1l; message_id = 2l; is_reply = false; payload = "hello" } in
...
let test_roundtrip_empty_payload () =
  with_socket_pair (fun reader writer ->
    let pkt = { stream_id = 0l; message_id = 1l; is_reply = false; payload = "" } in
    write_packet writer pkt;
    let pkt2 = read_packet reader in
...
let test_roundtrip_large_payload () =
  with_socket_pair (fun reader writer ->
    ...
    let pkt = { stream_id = 42l; message_id = 99l; is_reply = false; payload } in
    (* Write in a separate thread to avoid deadlock: the 65KB payload exceeds
         ...
         some data. *)
    let writer_thread = Thread.create (fun () -> write_packet writer pkt) () in
    let pkt2 = read_packet reader in
...
let test_roundtrip_max_stream_id () =
  with_socket_pair (fun reader writer ->
    let pkt =
      { stream_id = 0xFFFFFFFFl; message_id = 1l; is_reply = false; payload = "test" }
    in
    write_packet writer pkt;
    let pkt2 = read_packet reader in
...
let test_roundtrip_reply_bit () =
  with_socket_pair (fun reader writer ->
    let pkt = { stream_id = 5l; message_id = 10l; is_reply = true; payload = "reply" } in
    write_packet writer pkt;
    let pkt2 = read_packet reader in
...
let test_roundtrip_reply_bit_unset () =
  with_socket_pair (fun reader writer ->
    let pkt =
      { stream_id = 5l; message_id = 10l; is_reply = false; payload = "request" }
    in
    write_packet writer pkt;
    let pkt2 = read_packet reader in
...
let test_invalid_magic () =
  with_socket_pair (fun reader writer ->
    ...
    (try
       let (_ : Protocol.packet) = read_packet reader in
       ()
     with
     | Failure msg ->
       exn_raised := true;
...
let test_recv_exact_partial_close () =
  ...
  (try
     ...
     ()
   with
   | Connection_closed _ -> exn_raised := true);
  Core_unix.close reader;
...
let test_recv_exact_no_data_close () =
  ...
  (try
     ...
     ()
   with
   | Partial_packet _ -> exn_raised := true);
  Core_unix.close reader;
...
let test_pp_packet () =
  let pkt = { stream_id = 1l; message_id = 2l; is_reply = false; payload = "hi" } in
...
let test_equal_packet_different () =
  let pkt1 = { stream_id = 1l; message_id = 2l; is_reply = false; payload = "hi" } in
  let pkt2 = { stream_id = 1l; message_id = 2l; is_reply = true; payload = "hi" } in
  let pkt3 = { stream_id = 1l; message_id = 3l; is_reply = false; payload = "hi" } in
  let pkt4 = { stream_id = 2l; message_id = 2l; is_reply = false; payload = "hi" } in
  let pkt5 = { stream_id = 1l; message_id = 2l; is_reply = false; payload = "bye" } in
...
let tests =
  ...
  ; Alcotest.test_case "close_stream_message_id" `Quick test_close_stream_message_id
  ...
  ; Alcotest.test_case "CRC32 parts matches single" `Quick test_crc32_parts_matches_single
  ; Alcotest.test_case "CRC32 parts empty segments" `Quick test_crc32_parts_empty_segments
  ; (* Packet round-trip *)
    Alcotest.test_case "round-trip simple" `Quick test_roundtrip_simple
  ; Alcotest.test_case "round-trip empty payload" `Quick test_roundtrip_empty_payload
  ; Alcotest.test_case "round-trip large payload" `Quick test_roundtrip_large_payload
  ; Alcotest.test_case "round-trip max stream" `Quick test_roundtrip_max_stream_id
  ; Alcotest.test_case "round-trip reply bit set" `Quick test_roundtrip_reply_bit
  ; Alcotest.test_case "round-trip reply bit unset" `Quick test_roundtrip_reply_bit_unset
  ; (* Error cases *)
  ...
  ; Alcotest.test_case "recv_exact partial close" `Quick test_recv_exact_partial_close
  ; Alcotest.test_case "recv_exact no data close" `Quick test_recv_exact_no_data_close
  ; (* pp_packet, equal_packet *)
    Alcotest.test_case "pp_packet" `Quick test_pp_packet
  ; Alcotest.test_case "equal_packet different fields" `Quick test_equal_packet_different
  ; (* pack/unpack *)
