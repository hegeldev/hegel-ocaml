...
let test_one_of_basic_schema () =
  ...
  | Some s ->
    ...
    let children = Cbor_helpers.extract_list (List.assoc (`Text "generators") pairs) in
    let child_types =
      List.map
        (fun child ->
...
let test_ip_v4_schema () =
  ...
  | Some s ->
    ...
    Alcotest.(check string) "type" "ip_address" typ;
...
let tests =
  ...
  ; Alcotest.test_case "one_of dispatch bad format" `Quick test_one_of_dispatch_bad_format
  ...
  ; Alcotest.test_case "tuples2 basic transform" `Quick test_tuples2_basic_transform
  ...
  ; Alcotest.test_case "tuples3 basic transform" `Quick test_tuples3_basic_transform
  ...
  ; Alcotest.test_case "tuples4 basic transform" `Quick test_tuples4_basic_transform
