...
let test_double_handshake_send_raises () =
  ...
  (try ignore (send_handshake client_conn : string) with
   | Failure msg ->
     raised := true;
...
let test_new_stream_before_handshake_raises () =
  ...
  let raised = ref false in
  (try ignore (new_stream conn ()) with
...
let test_connect_stream_before_handshake_raises () =
  ...
  let conn = make_connection s1 ~name:"Test" () in
  let raised = ref false in
  (try ignore (connect_stream conn 1l ()) with
   | Failure _ -> raised := true);
  Alcotest.(check bool) "raised" true !raised;
  close conn
;;
...
let test_connect_stream_already_exists_raises () =
  ...
  handshake_pair peer_conn client_conn;
  let raised = ref false in
  (try ignore (connect_stream client_conn 0l ()) with
   | Failure msg ->
     raised := true;
     Alcotest.(check bool)
       "already connected"
...
let test_stream_timeout () =
  ...
  let raised = ref false in
  (try ignore (receive_request ch ~timeout:0.1 ()) with
   | Failure msg ->
     raised := true;
...
let test_stream_repr_no_role () =
  ...
  Alcotest.(check bool) "no role= in repr" true (not (contains_substring r "role="));
  close peer_conn;
  close client_conn
;;
...
let test_stream_name_no_role_with_conn_name () =
  ...
  Alcotest.(check bool) "no parens (no role)" true (not (contains_substring n "("));
  close peer_conn;
  close client_conn
;;
...
let test_request_response () =
              ...
              let y =
                Cbor_helpers.extract_int
                  (List.Assoc.find_exn
                     ...
                     (`Text "y"))
              in
              send_response_value ch msg_id (`Map [ `Text "sum", `Int (x + y) ])
            done
          with
          | Failure _ -> ());
         peer_done := true)
  ...
  let result =
    pending_get (request send_ch (`Map [ `Text "x", `Int 2; `Text "y", `Int 3 ]))
  in
  let sum =
    Cbor_helpers.extract_int
      (List.Assoc.find_exn
...
let test_request_error () =
  ...
  in
  let raised = ref false in
  (try ignore (result_or_error body) with
   | Request_error e ->
     raised := true;
...
let test_result_or_error_raises () =
  let body = `Map [ `Text "error", `Text "bad"; `Text "type", `Text "TestError" ] in
  let raised = ref false in
  (try ignore (result_or_error body) with
   | Request_error e ->
     raised := true;
...
let test_duplicate_response_error () =
  ...
  Queue.enqueue
    ch.inbox.queue
    (Pkt { stream_id = 0l; message_id = 42l; is_reply = true; payload = "second" });
  let raised = ref false in
  (try process_one_message ch () with
   | Failure msg ->
     raised := true;
     Alcotest.(check bool)
       "got two responses"
...
let test_close_stream_creates_dead_stream_with_connect () =
         ...
         let stream_id =
           Cbor_helpers.extract_int
             (List.Assoc.find_exn
                ...
                (`Text "stream"))
         in
         ignore (connect_stream peer_conn (Int32.of_int_exn stream_id) ~role:"Hello" ());
         send_response_value ch msg_id (`Text "Ok");
  ...
  (match Hashtbl.find peer_conn.streams client_ch.stream_id with
   | Some (Dead d) ->
     ...
     Alcotest.(check bool) "name has Hello" true (contains_substring d.name "Hello")
   | Some (Live _) -> Alcotest.fail "expected dead stream"
   | None -> Alcotest.fail "expected stream entry");
  close client_conn;
  close peer_conn
...
let test_debug_mode_recv () =
  ...
  write_packet s2 { stream_id = 0l; message_id = 1l; is_reply = false; payload = "hello" };
  Caml_unix.sleepf 0.1;
  close conn
;;
...
let test_reply_to_nonexistent_ignored () =
  ...
  send_packet
    client_conn
    { stream_id = 999l; message_id = 1l; is_reply = true; payload = "reply_data" };
...
let test_result_or_error_neither () =
  ...
  (try ignore (result_or_error body) with
   | Failure msg ->
     raised := true;
     Alcotest.(check bool) "has 'neither'" true (contains_substring msg "neither"));
  Alcotest.(check bool) "raised" true !raised
;;
...
let test_debug_close_already_dead_stream () =
  ...
  Hashtbl.set conn.streams ~key:50l ~data:(Dead { stream_id = 50l; name = "OldDead" });
  ...
  (match Hashtbl.find conn.streams 50l with
   | Some (Dead d) -> Alcotest.(check string) "dead name preserved" "OldDead" d.name
...
let test_debug_close_existing_live_stream () =
  ...
  let _ch_peer = connect_stream peer_conn ch_client.stream_id ~role:"LiveChSrv" () in
...
let test_send_handshake_short_response () =
  ...
  (try ignore (send_handshake client_conn : string) with
   | Failure msg ->
     raised := true;
     Alcotest.(check bool) "bad handshake" true (contains_substring msg "Bad handshake"));
  Thread.join t;
  Alcotest.(check bool) "raised" true !raised;
  close client_conn;
  close peer_conn
...
let test_control_stream_failure () =
  let s1, s2 = make_socket_pair () in
  let conn = make_connection s1 ~name:"Test" () in
  Hashtbl.set conn.streams ~key:0l ~data:(Dead { stream_id = 0l; name = "Control" });
  let raised = ref false in
  (try ignore (control_stream conn) with
   | Failure _ -> raised := true);
  Alcotest.(check bool) "raised" true !raised;
  close conn;
  Unix.close s2
...
let test_write_to_broken_socket_raises () =
  ...
  (try
     Protocol.write_packet
       s2
       { Protocol.stream_id = 0l; message_id = 1l; is_reply = false; payload = "hello" }
   with
   | Unix.Unix_error (Unix.EPIPE, _, _) -> raised := true);
...
let tests =
  ...
  ; Alcotest.test_case "connection double close" `Quick test_connection_double_close
  ; (* Handshake *)
    Alcotest.test_case
      "handshake returns version"
      `Quick
      test_send_handshake_returns_version
  ; Alcotest.test_case
      "double handshake send raises"
      `Quick
      test_double_handshake_send_raises
  ; Alcotest.test_case "bad handshake response" `Quick test_send_handshake_bad_response
  ; (* Stream allocation *)
    Alcotest.test_case
  ...
  ; Alcotest.test_case
      "stream close when conn not live"
      `Quick
      test_stream_close_when_connection_not_live
  ; Alcotest.test_case
      "process message when closed"
      `Quick
  ...
  ; Alcotest.test_case
      "stream name no role no conn name"
      `Quick
      test_stream_name_no_role_no_conn_name
  ; Alcotest.test_case
      "stream name with role no conn name"
      `Quick
      test_stream_name_with_role_no_conn_name
  ; Alcotest.test_case
      "stream name no role with conn name"
      `Quick
  ...
  ; Alcotest.test_case "pending request caching" `Quick test_pending_request_caching
  ; (* RequestError *)
    Alcotest.test_case "request error" `Quick test_request_error
  ; Alcotest.test_case
      "result_or_error returns result"
      `Quick
      test_result_or_error_returns_result
  ; Alcotest.test_case "result_or_error raises" `Quick test_result_or_error_raises
  ; (* Duplicate response *)
    Alcotest.test_case "duplicate response error" `Quick test_duplicate_response_error
  ; (* Shutdown *)
  ...
  ; Alcotest.test_case "message to dead stream" `Quick test_message_to_dead_stream
  ; Alcotest.test_case
      "close creates dead stream"
      `Quick
      test_close_stream_creates_dead_stream
  ; Alcotest.test_case
      "close creates dead stream with connect"
      `Quick
      test_close_stream_creates_dead_stream_with_connect
  ; (* Reader loop *)
    Alcotest.test_case "reader loop clean exit" `Quick test_reader_loop_clean_exit
  ; (* Packet dispatch *)
    Alcotest.test_case "process reply packet" `Quick test_process_reply_packet
  ; Alcotest.test_case "process request packet" `Quick test_process_request_packet
  ; (* Debug mode *)
    Alcotest.test_case "debug mode recv" `Quick test_debug_mode_recv
  ; Alcotest.test_case "debug close stream recv" `Quick test_debug_close_stream_recv
  ; (* Reply to nonexistent *)
    Alcotest.test_case
      "reply to nonexistent ignored"
      `Quick
  ...
  ; (* result_or_error edge cases *)
    Alcotest.test_case "error without type" `Quick test_result_or_error_error_without_type
  ; Alcotest.test_case "neither result nor error" `Quick test_result_or_error_neither
  ; (* entry_name *)
    Alcotest.test_case "entry_name live stream" `Quick test_entry_name_live_stream
  ; Alcotest.test_case "entry_name dead stream" `Quick test_entry_name_dead_stream
  ; (* Debug close for already-dead stream *)
    Alcotest.test_case
      "debug close already dead stream"
      `Quick
  ...
  ; (* Close with shutdown error *)
    Alcotest.test_case "close with shutdown error" `Quick test_close_with_shutdown_error
  ...
  ; Alcotest.test_case
      "wrong prefix handshake response"
      `Quick
      test_send_handshake_wrong_prefix
  ; (* control_stream failure *)
    Alcotest.test_case "control_stream failure" `Quick test_control_stream_failure
  ; (* Regression: rapid create/handshake/close must not hang *)
    Alcotest.test_case "rapid close does not hang" `Quick test_rapid_close_does_not_hang
  ; (* Regression: write to broken socket raises, not SIGPIPE *)
    Alcotest.test_case
      "write to broken socket raises EPIPE"
      `Quick
