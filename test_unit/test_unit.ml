...
let test_cbor_unsigned () =
  ...
  check "unsigned 23" (roundtrip (Hegel.Cbor.Unsigned 23) = Hegel.Cbor.Unsigned 23);
  check "unsigned 24" (roundtrip (Hegel.Cbor.Unsigned 24) = Hegel.Cbor.Unsigned 24);
  check "unsigned 255" (roundtrip (Hegel.Cbor.Unsigned 255) = Hegel.Cbor.Unsigned 255);
  check "unsigned 256" (roundtrip (Hegel.Cbor.Unsigned 256) = Hegel.Cbor.Unsigned 256);
  check "unsigned 65535" (roundtrip (Hegel.Cbor.Unsigned 65535) = Hegel.Cbor.Unsigned 65535);
  check "unsigned 65536" (roundtrip (Hegel.Cbor.Unsigned 65536) = Hegel.Cbor.Unsigned 65536);
  check "unsigned large" (roundtrip (Hegel.Cbor.Unsigned 1000000) = Hegel.Cbor.Unsigned 1000000)

let test_cbor_negative () =
  check "negative -1" (roundtrip (Hegel.Cbor.Negative (-1)) = Hegel.Cbor.Negative (-1));
  check "negative -10" (roundtrip (Hegel.Cbor.Negative (-10)) = Hegel.Cbor.Negative (-10));
  check "negative -100" (roundtrip (Hegel.Cbor.Negative (-100)) = Hegel.Cbor.Negative (-100));
  check "negative -1000" (roundtrip (Hegel.Cbor.Negative (-1000)) = Hegel.Cbor.Negative (-1000))

let test_cbor_text () =
  check "text empty" (roundtrip (Hegel.Cbor.Text "") = Hegel.Cbor.Text "");
  check "text hello" (roundtrip (Hegel.Cbor.Text "hello") = Hegel.Cbor.Text "hello");
  let long_str = String.make 300 'x' in
  check "text long" (roundtrip (Hegel.Cbor.Text long_str) = Hegel.Cbor.Text long_str)

let test_cbor_bytes () =
  check "bytes empty" (roundtrip (Hegel.Cbor.Bytes "") = Hegel.Cbor.Bytes "");
  check "bytes data" (roundtrip (Hegel.Cbor.Bytes "\x00\x01\x02") = Hegel.Cbor.Bytes "\x00\x01\x02")

let test_cbor_array () =
  check "array empty" (roundtrip (Hegel.Cbor.Array []) = Hegel.Cbor.Array []);
  check "array ints"
    (roundtrip (Hegel.Cbor.Array [ Hegel.Cbor.Unsigned 1; Hegel.Cbor.Unsigned 2 ])
...
let test_cbor_map () =
  ...
  check "map one pair"
    (roundtrip (Hegel.Cbor.Map [ (Hegel.Cbor.Text "key", Hegel.Cbor.Unsigned 42) ])
...
let test_cbor_float () =
  ...
  check "float -1.5" (roundtrip (Hegel.Cbor.Float (-1.5)) = Hegel.Cbor.Float (-1.5));
  check "float infinity" (roundtrip (Hegel.Cbor.Float infinity) = Hegel.Cbor.Float infinity);
...
let test_cbor_map_get () =
  let m = Hegel.Cbor.Map [ (Hegel.Cbor.Text "a", Hegel.Cbor.Unsigned 1); (Hegel.Cbor.Text "b", Hegel.Cbor.Unsigned 2) ] in
  check "map_get existing" (Hegel.Cbor.map_get m "a" = Some (Hegel.Cbor.Unsigned 1));
  check "map_get second" (Hegel.Cbor.map_get m "b" = Some (Hegel.Cbor.Unsigned 2));
  check "map_get missing" (Hegel.Cbor.map_get m "c" = None);
  check "map_get not a map" (Hegel.Cbor.map_get (Hegel.Cbor.Unsigned 5) "a" = None);
  (* map_get should skip non-Text keys *)
  let m2 = Hegel.Cbor.Map [ (Hegel.Cbor.Unsigned 1, Hegel.Cbor.Text "num"); (Hegel.Cbor.Text "a", Hegel.Cbor.Unsigned 1) ] in
  check "map_get skips non-text keys" (Hegel.Cbor.map_get m2 "a" = Some (Hegel.Cbor.Unsigned 1))

let test_cbor_map_get_exn () =
  let m = Hegel.Cbor.Map [ (Hegel.Cbor.Text "a", Hegel.Cbor.Unsigned 1) ] in
  check "map_get_exn existing" (Hegel.Cbor.map_get_exn m "a" = Hegel.Cbor.Unsigned 1);
...
let test_cbor_as_helpers () =
  ...
  check "as_bool some true" (Hegel.Cbor.as_bool (Hegel.Cbor.Bool true) = Some true);
  check "as_bool some false" (Hegel.Cbor.as_bool (Hegel.Cbor.Bool false) = Some false);
  check "as_bool none" (Hegel.Cbor.as_bool (Hegel.Cbor.Unsigned 1) = None);
  check "as_float from float" (Hegel.Cbor.as_float (Hegel.Cbor.Float 1.5) = Some 1.5);
  check "as_float from unsigned" (Hegel.Cbor.as_float (Hegel.Cbor.Unsigned 5) = Some 5.0);
  check "as_float from negative" (Hegel.Cbor.as_float (Hegel.Cbor.Negative (-3)) = Some (-3.0));
  ...
  check "as_int negative" (Hegel.Cbor.as_int (Hegel.Cbor.Negative (-5)) = Some (-5));
  check "as_int float integer" (Hegel.Cbor.as_int (Hegel.Cbor.Float 3.0) = Some 3);
  check "as_int float non-integer" (Hegel.Cbor.as_int (Hegel.Cbor.Float 3.5) = None);
  check "as_int text" (Hegel.Cbor.as_int (Hegel.Cbor.Text "x") = None)

let test_cbor_to_diagnostic () =
  check "diag unsigned" (Hegel.Cbor.to_diagnostic (Hegel.Cbor.Unsigned 42) = "42");
  check "diag negative" (Hegel.Cbor.to_diagnostic (Hegel.Cbor.Negative (-5)) = "-5");
  check "diag text" (Hegel.Cbor.to_diagnostic (Hegel.Cbor.Text "hi") = "\"hi\"");
  check "diag bytes" (Hegel.Cbor.to_diagnostic (Hegel.Cbor.Bytes "\x00\xff") = "h'00ff'");
  check "diag array" (Hegel.Cbor.to_diagnostic (Hegel.Cbor.Array [ Hegel.Cbor.Unsigned 1; Hegel.Cbor.Unsigned 2 ]) = "[1, 2]");
  check "diag map"
  ...
  check "diag tag" (Hegel.Cbor.to_diagnostic (Hegel.Cbor.Tag (258, Hegel.Cbor.Array [])) = "258([])");
...
let test_gen_string_contains () =
  check "string_contains found" (Hegel.Gen.string_contains "hello world" "world");
  check "string_contains not found" (not (Hegel.Gen.string_contains "hello world" "xyz"));
  check "string_contains needle longer" (not (Hegel.Gen.string_contains "hi" "hello"));
  ...
  check "string_contains exact match" (Hegel.Gen.string_contains "hello" "hello");
...
let test_gen_cbor_helpers () =
  (* cbor_int_value *)
  check "cbor_int_value unsigned" (Hegel.Gen.cbor_int_value (Hegel.Cbor.Unsigned 42) = 42);
  check "cbor_int_value negative" (Hegel.Gen.cbor_int_value (Hegel.Cbor.Negative (-5)) = -5);
  check "cbor_int_value float-as-int" (Hegel.Gen.cbor_int_value (Hegel.Cbor.Float 3.0) = 3);
  check_raises_failure "cbor_int_value invalid" (fun () ->
    ignore (Hegel.Gen.cbor_int_value (Hegel.Cbor.Text "x")));
  (* cbor_float_value *)
  check "cbor_float_value float" (Hegel.Gen.cbor_float_value (Hegel.Cbor.Float 1.5) = 1.5);
  check "cbor_float_value unsigned" (Hegel.Gen.cbor_float_value (Hegel.Cbor.Unsigned 5) = 5.0);
  check "cbor_float_value negative" (Hegel.Gen.cbor_float_value (Hegel.Cbor.Negative (-3)) = -3.0);
  check_raises_failure "cbor_float_value invalid" (fun () ->
    ignore (Hegel.Gen.cbor_float_value (Hegel.Cbor.Text "x")));
  (* cbor_text_value *)
  check "cbor_text_value text" (Hegel.Gen.cbor_text_value (Hegel.Cbor.Text "hi") = "hi");
  check_raises_failure "cbor_text_value invalid" (fun () ->
    ignore (Hegel.Gen.cbor_text_value (Hegel.Cbor.Unsigned 1)));
  (* cbor_bool_value *)
  check "cbor_bool_value true" (Hegel.Gen.cbor_bool_value (Hegel.Cbor.Bool true) = true);
  check "cbor_bool_value false" (Hegel.Gen.cbor_bool_value (Hegel.Cbor.Bool false) = false);
...
let test_gen_one_of_empty () =
  check_raises_failure "one_of empty list" (fun () -> ignore (Hegel.Gen.one_of []))
...
let test_protocol_receive_response () =
  ...
  Fun.protect
    ~finally:(fun () -> Unix.close rd1; Unix.close wr1)
    (fun () ->
  ...
  Fun.protect
    ...
    (fun () ->
      ...
      | 0 ->
        ...
        let error_response = Hegel.Cbor.encode_to_string
          (Hegel.Cbor.Map [
            (Hegel.Cbor.Text "error", Hegel.Cbor.Text "something went wrong");
      ...
      | pid ->
        check_raises_failure "request_cbor error response" (fun () ->
          ignore (Hegel.Protocol.Channel.request_cbor ch_send (Hegel.Cbor.Map [])));
  ...
  Fun.protect
    ...
    (fun () ->
      ...
      | 0 ->
        ...
        let pkt = Hegel.Protocol.read_packet rd in
        let response = Hegel.Cbor.encode_to_string
          (Hegel.Cbor.Map [
        ...
        exit 0
      | pid ->
        let result = Hegel.Protocol.Channel.request_cbor ch_send (Hegel.Cbor.Map []) in
...
let test_extract_channel_id () =
  let valid = Hegel.Cbor.Map [
    (Hegel.Cbor.Text "channel", Hegel.Cbor.Unsigned 42);
  ] in
  ...
  let wrong_type = Hegel.Cbor.Map [
    (Hegel.Cbor.Text "channel", Hegel.Cbor.Text "not an int");
  ] in
...
let test_start_span_aborted () =
  Hegel.State.set_test_aborted true;
  check_raises "start_span when aborted" (fun () ->
    Hegel.Gen.start_span 1);
  Hegel.State.set_test_aborted false
...
let with_mock_server test_fn server_fn =
  ...
  | 0 ->
    ...
    (try
       while true do
         let pkt = Hegel.Protocol.read_packet rd in
         let ok = Hegel.Cbor.encode_to_string
           (Hegel.Cbor.Map [(Hegel.Cbor.Text "result", Hegel.Cbor.Null)]) in
         Hegel.Protocol.write_packet rd {
           Hegel.Protocol.channel = pkt.channel;
           message_id = pkt.message_id;
           is_reply = true;
...
let test_start_span_stop_test () =
  with_mock_server
    (fun () ->
      ...
      check_raises "start_span StopTest" (fun () ->
        Hegel.Gen.start_span 1);
...
let respond_ok_then_custom rd n_ok custom_result =
  ...
  let pkt = Hegel.Protocol.read_packet rd in
  let resp = Hegel.Cbor.encode_to_string
    (Hegel.Cbor.Map [(Hegel.Cbor.Text "result", custom_result)]) in
  Hegel.Protocol.write_packet rd {
    Hegel.Protocol.channel = pkt.channel;
    message_id = pkt.message_id;
    is_reply = true;
  ...
  with_mock_server
    (fun () ->
      check_raises_failure "new_collection non-text" (fun () ->
        let gen = Hegel.Gen.list ~min_size:1 ~max_size:3
          (Hegel.Gen.filter (fun x -> x > 0) (Hegel.Gen.int ~min:(-10) ~max:10 ())) in
        ignore (gen.generate ())))
    (fun rd ->
  ...
  with_mock_server
    ...
    (fun rd ->
      let respond (pkt : Hegel.Protocol.packet) result =
        let resp = Hegel.Cbor.encode_to_string
          (Hegel.Cbor.Map [(Hegel.Cbor.Text "result", result)]) in
        Hegel.Protocol.write_packet rd {
          Hegel.Protocol.channel = pkt.channel;
          message_id = pkt.message_id;
          is_reply = true;
...
let test_non_stop_test_error_exits_134 () =
  ...
  | pid ->
    ...
    | Unix.WEXITED 134 -> ()
    | Unix.WEXITED n ->
      Printf.eprintf "FAIL: non-StopTest error expected exit 134, got %d\n%!" n;
      incr failures
    | _ ->
