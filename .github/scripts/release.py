...
def set_dune_version(dune_project: Path, new_version: str) -> None:
    ...
    dune_project.write_text(new_text)


def set_opam_version(opam_file: Path, new_version: str) -> None:
...
def release() -> None:
    ...
    m = re.search(r"\(version\s+([^)]+)\)", dune_project.read_text())
    new_version = bump_version(m.group(1), release_type)

    set_dune_version(dune_project, new_version)
    set_opam_version(ROOT / "hegel.opam", new_version)
    set_opam_version(ROOT / "ppx_hegel.opam", new_version)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Release automation for hegel-ocaml.")
