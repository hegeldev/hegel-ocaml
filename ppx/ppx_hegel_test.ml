(** PPX expander for [let%hegel_test ...].
    ...
    The [@@settings ...] and [@@blobs ...] attributes are both optional. The
    [@@blobs ...] attribute drives the failure-blob workflow: an empty list
    [[@@blobs []]] enables recording mode (a failure prints the captured
...
(** [extract_blobs_attr attrs] returns the parsed string list if a
    [[@@blobs ...]] attribute is present, else [None]. *)
let extract_blobs_attr (attrs : attributes) : string list option =
  List.find_map
    (fun (attr : attribute) ->
...
(** [build_items ~loc ~function_name ~settings_expr ~blobs ~body_fn] returns
    the pair of structure items the expander splices in:

    {[
      let function_name () =
        Hegel.Session.run_hegel_test
          [?settings] location [?blobs] body_fn
      ;;

      let () =
        Hegel_test_runtime.register ~name:.. ~file:.. ~line:.. function_name
    ]} *)
let build_items ~loc ~function_name ~settings_expr ~blobs ~body_fn : structure_item list =
  ...
  let call =
    ...
    | Some bs ->
      ...
      let blobs_e = Ast_builder.Default.elist ~loc elements in
      [%expr [%e base_call] ~blobs:[%e blobs_e] [%e body_fn]]
...
let expand_value_binding ~loc (vb : value_binding) : structure_item list =
  ...
  let blobs = extract_blobs_attr vb.pvb_attributes in
  (* The body of [let%hegel_test name <args> = expr] is parsed as
     [let name = <args -> expr>]. We pass that lambda as the [test_fn] to
     [Hegel.Session.run_hegel_test]. *)
  build_items ~loc ~function_name ~settings_expr ~blobs ~body_fn:vb.pvb_expr
;;
