...
let eapply f args =
  List.fold_left (fun acc arg -> [%expr [%e acc] [%e arg]]) f args
...
let rec gen_for_core_type ct =
  ...
  | Ptyp_var tv -> evar (gen_name_for_var tv)
  | Ptyp_constr ({ txt = lident; _ }, args) -> (
      ...
      | _ ->
          ...
          if gen_args = [] then base else eapply base gen_args)
  | Ptyp_tuple [ a; b ] ->
      [%expr Hegel.Gen.pair [%e gen_for_core_type a] [%e gen_for_core_type b]]
  | Ptyp_tuple [ a; b; c ] ->
      [%expr
        Hegel.Gen.triple [%e gen_for_core_type a] [%e gen_for_core_type b]
          [%e gen_for_core_type c]]
  | Ptyp_tuple (a :: rest) ->
      let rest_type = { ct with ptyp_desc = Ptyp_tuple rest } in
      [%expr
        Hegel.Gen.pair [%e gen_for_core_type a] [%e gen_for_core_type rest_type]]
  | _ -> Location.raise_errorf ~loc:ct.ptyp_loc "ppx_hegel: unsupported type"
...
let gen_for_record fields =
  ...
  let bind_field_gens body =
    List.fold_right
      (fun (fname, gen_expr) acc ->
        [%expr
          let [%p pvar ("__gen_" ^ fname)] = [%e gen_expr] in
          [%e acc]])
      field_gens body
  in
  ...
  let build_record var_prefix =
    let fields =
      List.map
        (fun fname -> ({ txt = Lident fname; loc }, evar (var_prefix ^ fname)))
        field_names
    in
  ...
  let as_basic_expr =
    ...
    let parse_body =
      ...
      let let_bindings =
        List.mapi
          (fun i fname ->
            let parse_expr =
              [%expr
                [%e evar ("__basic_" ^ fname)].Hegel.Gen.parse
      ...
      List.fold_right
        (fun (fname, expr) acc ->
          [%expr
            let [%p pvar ("__v_" ^ fname)] = [%e expr] in
            [%e acc]])
        let_bindings (build_record "__v_")
    in
            ...
            Hegel.Gen.schema =
              Hegel.Cbor.Map
                [
                  ...
                  ( Hegel.Cbor.Text "elements",
                    Hegel.Cbor.Array [%e schema_array] );
                ];
            Hegel.Gen.parse =
              (fun __raw ->
    ...
    List.fold_right
      (fun (fname, expr) acc ->
        [%expr
          let [%p pvar ("__v_" ^ fname)] = [%e expr] in
          [%e acc]])
      field_bindings (build_record "__v_")
  in
  ...
  let generate_body =
    [%expr
      ...
      | Some __basic ->
          __basic.Hegel.Gen.parse
            (Hegel.Gen.generate_raw __basic.Hegel.Gen.schema)
      | None ->
          Hegel.Gen.group Hegel.Gen.Labels.fixed_dict (fun () ->
              [%e generate_fallback])]
  in
      ...
      (fun cd ->
        ...
        | Pcstr_tuple [ t ] ->
            ...
            let mapper =
              [%expr
                fun __x ->
                  [%e
                    Ast_builder.Default.pexp_construct ~loc ctor
                      (Some (evar "__x"))]]
            in
        ...
        | Pcstr_tuple ts ->
            ...
            let tuple_gen =
              ...
              | [ a; b ] ->
                  [%expr
                    Hegel.Gen.pair [%e gen_for_core_type a]
                      [%e gen_for_core_type b]]
              | [ a; b; c ] ->
                  [%expr
                    Hegel.Gen.triple [%e gen_for_core_type a]
                      [%e gen_for_core_type b] [%e gen_for_core_type c]]
              | _ ->
                  let rec nest = function
                    | [ t ] -> gen_for_core_type t
                    | t :: rest ->
                        [%expr
                          Hegel.Gen.pair [%e gen_for_core_type t] [%e nest rest]]
                    | [] -> assert false
                  in
                  nest ts
            in
            ...
            let tuple_pat =
              ...
              | 3 ->
                  [%pat? [%p pvar "__a0"], [%p pvar "__a1"], [%p pvar "__a2"]]
              | _ ->
                  let rec nest_pat = function
                    | [ v ] -> pvar v
                    | v :: rest ->
                        Ast_builder.Default.ppat_tuple ~loc
                          [ pvar v; nest_pat rest ]
                    | [] -> assert false
                  in
                  nest_pat vars
            in
        ...
        | Pcstr_record fields ->
            ...
            let record_fields =
              List.map2
                (fun fname vname -> ({ txt = Lident fname; loc }, evar vname))
                field_names vars
            in
            let record_expr =
...
let type_is_self_referencing td =
  ...
  let iterator =
    object
      ...
      method! core_type ct =
        (match ct.ptyp_desc with
        | Ptyp_constr ({ txt = Lident n; _ }, _) when n = name -> found := true
        | _ -> ());
        super#core_type ct
    end
  in
...
let body_for_type_decl td =
  ...
  | _ ->
      Location.raise_errorf ~loc:td.ptype_loc "ppx_hegel: unsupported type kind"
...
let str_type_decl ~ctxt (rec_flag, tds) =
  ...
  List.map
    (fun td ->
      ...
      if is_rec && params = [] && type_is_self_referencing td then
        ...
        [%stri
          let [%p pvar fname] =
            ...
            let [%p pvar fname] =
              {
                Hegel.Gen.generate =
                  (fun () -> ![%e evar ref_name].Hegel.Gen.generate ());
                Hegel.Gen.as_basic = (fun () -> None);
              }
            in
            ignore [%e evar fname];
  ...
  List.map
    (fun td ->
      ...
      Ast_builder.Default.psig_value ~loc
        (Ast_builder.Default.value_description ~loc ~name:{ txt = fname; loc }
           ~type_:full_type ~prim:[]))
    tds
...
let () =
  ...
  Deriving.add "hegel" ~str_type_decl ~sig_type_decl |> Deriving.ignore
