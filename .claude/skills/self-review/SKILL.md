...
- **Hardcoded paths**: `/tmp/foo` in tests should be a generated temp directory for isolation and cleanup.
- **Missing error context**: error messages should help the user fix the problem (e.g., include "Install X manually: <url>").

## Coverage check

Run `just check-tests` and review the output. If new code is uncovered, address it before the PR — don't rely on CI to catch it. See the `coverage` skill for how to approach coverage gaps.
