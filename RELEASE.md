...
- Adds `Hegel.Conformance.with_overrun_metric` and `Hegel.Conformance.run_conformance_test` helpers. `with_overrun_metric` writes a placeholder client-side metric on `Data_exhausted` so the conformance binaries' per-test-case client metric count stays aligned with the new per-test-case server metric count required by hegel-core 0.4.13+. `run_conformance_test` is a thin wrapper around `Session.run_hegel_test` that applies that wrapping automatically; the conformance binaries now call it instead of the lower-level pair.
- Adds the `hegel-core-release` receiver workflow (`.github/workflows/bump-hegel-core.yml` and `.github/scripts/bump_hegel_core.py`) so future hegel-core releases will automatically open a bump PR here, mirroring the setup already in place for hegel-rust. The bump script also keeps the `nix/flake.nix` reference and the conformance-harness pin in `justfile` in sync with `lib/session.ml`.
