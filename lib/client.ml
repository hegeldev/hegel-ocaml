    ...
    | Test_run ->
      ...
      then (
        let blobs =
          ...
          | _ -> failwith "Server did not return failure blobs"
        in
        match blobs with
        | [] -> ()
        | _ ->
          Stdlib.print_endline "[hegel] failure blob(s) recorded:";
          List.iter blobs ~f:(fun b -> Printf.printf "  - %S\n" b);
          Printf.printf
            "[hegel] to replay, add to your test: [@@blobs [ %s ]]\n%!"
