...
module Encode = struct
  ...
  let put b ~maj n =
    ...
    if n < 24 then init b ~maj n
      ...
      init b ~maj 25;
      put_n b 2 BE.set_int16 n
    end
    else if n <= max_uint32 then begin
      init b ~maj 26;
      put_n b 4 BE.set_int32 @@ Int32.of_int n
    end
    else begin
      init b ~maj 27;
      put_n b 8 BE.set_int64 @@ Int64.of_int n
    end
...
module Simple = struct
  ...
  let encode item =
    ...
    let rec write = function
      ...
      | `Simple n when (n >= 0 && n <= 23) || (n >= 32 && n <= 255) ->
          put b ~maj:7 n
  ...
  let rec extract_pair r =
    let a = extract r in
    let b =
      try extract r with Break -> fail "extract_pair: unexpected break"
    in
    (a, b)
  ...
  let decode_partial s =
    let i = ref 0 in
    let x =
      try extract (s, i) with Break -> fail "decode: unexpected break"
    in
  ...
  let decode s : t =
    ...
    if rest = "" then x
      ...
      fail "decode: extra data: len %d pos %d" (String.length s)
