(* Vendored from https://github.com/mirage/ocaml-cbor (v0.5).
   ...
   Local modifications: rewrote [Simple.to_diagnostic] to avoid escaping
   closures over locally-allocated state, for compatibility with the OxCaml
   mode/locality checker. *)
...
(* Patched for OxCaml: the upstream version uses [let put = Buffer.add_string b]
   and [list_iteri (fun ...)], where the inner lambda captures the local [put]
   closure. OxCaml's mode checker rejects this because the lambda passed to
   [list_iteri] is inferred as global but [put] is local. We inline
   [Buffer.add_string b] and replace the [list_iteri] calls with mutually
   recursive helpers that never escape the enclosing region. *)
let to_diagnostic item =
  ...
  | `Undefined -> Buffer.add_string b "undefined"
  | `Int n -> bprintf b "%d" n
  | `Float f ->
    begin match classify_float f with
    | FP_nan -> Buffer.add_string b "NaN"
    | FP_infinite -> Buffer.add_string b (if f < 0. then "-Infinity" else "Infinity")
    | FP_zero | FP_normal | FP_subnormal -> Buffer.add_string b (Float.to_string f)
    end
  ...
  | `Tag (t, v) ->
    ...
    write v;
    Buffer.add_string b ")"
  and write_array = function
    | [] -> ()
    | [x] -> write x
    | x :: rest -> write x; Buffer.add_string b ", "; write_array rest
  and write_map = function
    | [] -> ()
    | [(k, v)] -> write k; Buffer.add_string b ": "; write v
    | (k, v) :: rest ->
      write k; Buffer.add_string b ": "; write v;
      Buffer.add_string b ", "; write_map rest
  in
  write item;
  Buffer.contents b
